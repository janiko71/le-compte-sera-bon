...
Côté performances, la solution est très souvent trouvée en moins de 2 secondes, mais le calcul de toutes les possibilités peut prendre plus d'une minute, hélas. 
...
Nombre recherché : 611

Meilleure solution possible : 610 = (((1 + (6 x 10)) x (7 + (1 + 2))))
...
Donc, par défi, j'ai voulu voir comment résoudre le problème et surtout en combien de temps. L'algorithme n'est pas spécialement optimisé, mais il tourne sur un smartphone, en Python, en moins d'une seconde... Un problème à la combinatoire de ce niveau, impossible à résoudre rapidement il y a encore quelques années, et aujourd'hui largement à la portée de mon smartphone (à vérifier avec l'algo corrigé).
